package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2350:int = 13421772;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 != null)
         {
            ...
            {
               this.var_1955 = Math.max(this.var_1955,_loc6_.colorLayerIndex);
            }
         }
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_748 != null && !this.var_1956)
            {
               _loc3_ = this.var_748;
            }
            else
            ...
            if(this.var_845)
            {
               switch(this.var_845.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_874;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_1024;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1478.color = this.const_2350;
         }
         else
         {
            this.var_1478.color = this.const_2351;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_157;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_672 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1957)
               {
                  ...
                  this.var_1957 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_672.width,this.var_672.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_672.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_672.y;
               if(this.var_1956 && _loc2_.colorLayerIndex > 0)
               {
