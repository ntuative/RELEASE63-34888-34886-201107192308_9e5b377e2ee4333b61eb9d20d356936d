package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2357:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_1087 = new Timer(this.const_2357,1);
         this.var_1087.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_494)
         {
            ...
            this.var_494.addEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_494)
         {
            ...
            this.var_494.removeEventListener(AssetLoaderEvent.const_42,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1088 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1356);
         ...
         else
         {
            ...
            if(this.var_1356.indexOf("?") > 0)
            {
               _loc2_ = this.var_1356 + "&retry=" + this.var_1088;
            }
            else
            {
               _loc2_ = this.var_1356 + "?retry=" + this.var_1088;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2888,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1358[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1358[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2084)
         {
            this.var_1357.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1086)
         {
            _loc7_ = true;
            _loc4_ = this.var_1086[_loc3_];
         ...
         {
            delete this.var_1086[_loc3_];
         }
         while(_loc6_ < this.var_887.length)
         {
            _loc5_ = this.var_887[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_887.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1087.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_886.length > 0 && this.var_887.length < this.const_1869)
         {
            _loc1_ = this.var_886[0];
            _loc1_.startDownloading();
            this.var_887.push(this.var_886.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_886.indexOf(param1) == -1 && this.var_887.indexOf(param1) == -1)
         {
