package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_812:String = "InitConnection";
       
      
      private var var_128:HabboCommunicationDemo;
      ...
      {
         this.var_128.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_128 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_128.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_157,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_128.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         if(_loc8_)
         {
            if(this.var_128.habboConfiguration)
            {
               if(this.var_128.habboConfiguration.getKey("use.sso","false") == "true")
               {
                  _loc8_.select();
               }
            }
         ...
         if(this.var_128.habboConfiguration && this.var_128.habboConfiguration.getKey("try.existing.session","false") == "true")
         {
            this.var_1506 = true;
            this._window.visible = false;
            this.windowEventProcessor(WindowEvent.allocate(WindowEvent.const_175,this._window,null,false));
         }
      }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_128.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_128.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         {
            dispatchEvent(new Event(const_812));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_128.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_155.removeEventListener(SSOTicket.const_462,this.onParsedTicketSuccess);
            this.var_155.removeEventListener(SSOTicket.const_377,this.onParsedTicketFailure);
            this.var_155 = null;
         }
         var _loc2_:String = "";
         if(this.var_128.habboConfiguration != null)
         {
            _loc2_ = this.var_128.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_128.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_155 = new SSOTicket(this.var_128.assets,_loc3_,_loc2_,param1);
         this.var_155.addEventListener(SSOTicket.const_462,this.onParsedTicketSuccess);
         this.var_155.addEventListener(SSOTicket.const_377,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_128.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_128.flashClientUrl = this.var_155.flashClientUrl;
         this.var_128.ssoTicket = this.var_155.ticket;
         this.var_155.removeEventListener(SSOTicket.const_462,this.onParsedTicketSuccess);
         this.var_155.removeEventListener(SSOTicket.const_377,this.onParsedTicketFailure);
         this.var_155 = null;
         dispatchEvent(new Event(const_812));
