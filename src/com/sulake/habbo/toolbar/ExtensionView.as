package com.sulake.habbo.toolbar
{
   ...
   public class ExtensionView implements IExtensionView
   {
      ...
      private var var_135:IItemListWindow;
      ...
      public function ExtensionView(param1:IHabboWindowManager, param2:IAssetLibrary, param3:HabboToolbar)
      {
         ...
         if(_loc4_)
         {
            this.var_135 = param1.buildFromXML(_loc4_.content as XML,1) as IItemListWindow;
         }
         var _loc5_:IWindowContainer = this.var_135.desktop;
         if(this.var_135)
         {
            this.var_135.x = _loc5_.width - this.var_135.width - const_1134;
            this.var_135.y = const_1134;
            this.var_135.visible = true;
         }
         else
         {
            Core.error("Unable to initialize Toolbar Extension view window from xml asset",false,Core.const_1367);
         }
         this._items = new Map();
      }
      ...
      public function get screenHeight() : uint
      {
         ...
         return this.var_135.height + this.var_135.y;
      }
      ...
      public function attachExtension(param1:String, param2:IWindow, param3:int = -1) : void
      {
         ...
         if(this.var_135)
         {
            if(param3 == -1)
            {
               this.var_135.addListItem(param2);
            }
            else
            {
               this.var_135.addListItemAt(param2,param3);
            }
            this.var_135.invalidate();
         }
      ...
      public function detachExtension(param1:String) : void
      {
         ...
         if(this.var_135)
         {
            this.var_135.removeListItem(_loc2_);
         }
      ...
      private function onResizeTimer(param1:TimerEvent) : void
      {
         if(this._toolbar)
         {
            this._toolbar.events.dispatchEvent(new ExtensionViewEvent(ExtensionViewEvent.const_1193));
         }
      }
   }
}
