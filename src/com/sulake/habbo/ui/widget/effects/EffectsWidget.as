package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1483:int = 320;
      
      private static const const_1484:int = 48;
      
      private static const const_1485:int = 2;
      ...
      {
         return var_939 as EffectsWidgetHandler;
      }
      ...
      public function open() : void
      {
         ...
         if(!this._view)
         {
            ...
            this._view.x = _loc2_.right + const_1485;
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_377.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_377.add(_loc3_.type,_loc1_);
               this.var_65.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_377.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_377.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_65.removeListItem(_loc1_.window);
               this.var_377.remove(this.var_377.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_65.scrollableRegion.height;
         this.var_65.height = Math.max(Math.min(_loc5_,const_1483),const_1484);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1313,true);
      }
   }
}
