package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1670:String = "asset-libraries";
      
      private static const const_1666:String = "library";
      
      private static const const_1662:String = "service-libraries";
      
      private static const const_1663:String = "library";
      
      private static const const_1668:String = "component-libraries";
      
      private static const const_1665:String = "library";
      
      private static const const_1667:String = "error_data";
      
      private static const const_1669:String = "error_cat";
      
      private static const const_1664:String = "error_desc";
       
      
      private var var_404:LibraryLoaderQueue;
      
      private var var_878:IEventDispatcher;
      
      private var var_571:uint;
      
      private var var_403:Function;
      
      private var var_229:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_959:
               ...
               this.var_229 = new Profiler(this);
               attachComponent(this.var_229,[new IIDProfiler()]);
               var_432.addChild(new ProfilerViewer(this.var_229));
               break;
            case Core.const_617:
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_297[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1667,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1669,String(param3));
         ErrorReportStorage.setParameter(const_1664,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_878 = loadingEventDelegate;
         if(this.var_404 == null)
         {
         ...
         try
         {
            node = config.child(const_1670)[0];
            if(node != null)
            {
               list = node.child(const_1666);
         ...
         try
         {
            node = config.child(const_1662)[0];
            if(node != null)
            {
               list = node.child(const_1663);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1668)[0];
            if(node != null)
            {
               list = node.child(const_1665);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_878)
         {
            this.var_878.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_878)
         {
            ...
            this.var_878.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_878)
                  {
                     this.var_878.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_571 - this.var_404.length,this.var_571,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_229 ? 0 : this.var_1614 & 0;
         if(_loc3_ == Core.const_617)
         {
            this.var_297[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_297[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_229 ? 0 : this.var_1614 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_297[_loc5_] as Array;
            if(_loc4_ == Core.const_617)
            {
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_701[priority] = 0;
            receivers = this.var_297[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            {
               if(this.var_701[priority] < priority)
               {
                  ++this.var_701[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_229.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_229)
            {
               detachComponent(this.var_229);
               this.var_229.dispose();
               this.var_229 = null;
            }
            switch(this.var_1614 & 0)
            {
               case Core.const_1244:
                  this.var_403 = this.simpleFrameUpdateHandler;
                  break;
