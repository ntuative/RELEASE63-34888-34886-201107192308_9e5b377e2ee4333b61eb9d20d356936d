package com.sulake.core.window.utils
{
   ...
   public class TextStyleManager
   {
      ...
      public static function setStyles(param1:Array, param2:Boolean = false) : void
      {
         ...
         if(param2)
         {
            _loc5_ = [var_99[const_205],var_99[ITALIC],var_99[BOLD]];
            var_99.reset();
            var_99[const_205] = _loc5_[0];
         ...
         for each(_loc4_ in param1)
         {
            ...
            if(var_566.indexOf(_loc4_.name) == -1)
            {
               var_566.push(_loc4_.name);
            }
         }
         _eventDispatcher.dispatchEvent(new Event(Event.CHANGE));
      ...
      private static function parseStyleNamesFromCSS(param1:String) : Array
      {
         ...
         var _loc4_:Array = _loc3_.split(const_714);
         if(countSubStrings(param1,const_713) != countSubStrings(param1,const_714))
         {
            throw new Error("Mismatching amount of \"" + const_713 + "\" versus \"" + const_714 + "\", please check the CSS!");
         }
         for each(_loc5_ in _loc4_)
         {
            while(_loc5_.indexOf(const_1637) == 0)
            {
               _loc5_ = _loc5_.substring(_loc5_.indexOf(const_1638) + 2,_loc5_.length);
            }
            _loc5_ = _loc5_.slice(0,_loc5_.indexOf(const_713)).split(" ").join("");
            if(_loc5_.length)
            {
               _loc2_.push(_loc5_);
            }
